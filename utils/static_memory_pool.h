...
constexpr size_t POOL_PADDING = 1024;
...
public:
    LocalMemoryPool(size_t internal_size, size_t leaf_size, size_t leaf_pool_size, size_t internal_pool_size) :
        ...
        internalSlotSize(ALIGNED_SIZE(sizeof(SlotMeta)) + ALIGNED_SIZE(internal_size)),
        poolSize(POOL_PADDING + ALIGNED_SIZE(leaf_pool_size, leafSlotSize) + ALIGNED_SIZE(internal_pool_size, internalSlotSize) + POOL_PADDING),
        internalBeg(POOL_PADDING), mid(POOL_PADDING + ALIGNED_SIZE(internal_pool_size, internalSlotSize)),
        leafEnd(POOL_PADDING + ALIGNED_SIZE(internal_pool_size, internalSlotSize) + ALIGNED_SIZE(leaf_pool_size, leafSlotSize)),
        freeLeafSlots(ALIGNED_SIZE(leaf_pool_size, leafSlotSize) / leafSlotSize),
        freeInternalSlots(ALIGNED_SIZE(internal_pool_size, internalSlotSize) / internalSlotSize) {

        FatalAssert(leaf_size > 0, LOG_TAG_MEMORY, "leaf_size must be greater than 0");
        FatalAssert(poolSize > 0, LOG_TAG_MEMORY, "pool_size must be greater than 0");
#ifdef USE_HUGETLB
        ...
        internalBeg += reinterpret_cast<uintptr_t>(base);
        mid += reinterpret_cast<uintptr_t>(base);
        leafEnd += reinterpret_cast<uintptr_t>(base);

        nextAvailableInternalSlot.store(internalBeg, std::memory_order_relaxed);
        nextAvailableLeafSlot.store(mid, std::memory_order_relaxed);
    ...
    ~LocalMemoryPool() override {
        SANITY_CHECK(
            for (Slot* internalSlot = reinterpret_cast<Slot*>(internalBeg);
                 internalSlot != reinterpret_cast<Slot*>(nextAvailableInternalSlot.load(std::memory_order_relaxed));
                 internalSlot = reinterpret_cast<Slot*>(reinterpret_cast<uintptr_t>(internalSlot) + internalSlotSize)) {
                FatalAssert(internalSlot->meta.in_use == 0, LOG_TAG_MEMORY,
                            "MemoryPool is being destroyed while some slots are in use");
            }

            for (Slot* leafSlot = reinterpret_cast<Slot*>(mid);
                 leafSlot != reinterpret_cast<Slot*>(nextAvailableLeafSlot.load(std::memory_order_relaxed));
    ...
    void* AllocateInternal(AllocationFlags flags = {.value = 0}) {
        Slot* slot_ptr = nullptr;
        bool success = freeInternalSlots.TryPopHead(slot_ptr);
        if (!success) {
            slot_ptr = reinterpret_cast<Slot*>(nextAvailableInternalSlot.fetch_add(internalSlotSize));
            if (reinterpret_cast<uintptr_t>(slot_ptr) + internalSlotSize > mid) {
                nextAvailableInternalSlot.fetch_sub(internalSlotSize);
                slot_ptr = nullptr;
            } else {
                success = true;
            }
        } SANITY_CHECK( else {
            FatalAssert(slot_ptr->meta.type == static_cast<uint8_t>(SlotType::Internal), LOG_TAG_MEMORY,
        ...
        if (!success && !flags.non_blocking) {
            while (!success) {
                ...
                if (!success && nextAvailableInternalSlot.load(std::memory_order_acquire) + internalSlotSize <= mid) {
                    slot_ptr = reinterpret_cast<Slot*>(nextAvailableInternalSlot.fetch_add(internalSlotSize));
                    if (reinterpret_cast<uintptr_t>(slot_ptr) + internalSlotSize > mid) {
                        nextAvailableInternalSlot.fetch_sub(internalSlotSize);
                        slot_ptr = nullptr;
...
#ifdef MEMORY_DEBUG
        {
            ...
            AllocationSanityCheck((void*)(slot_ptr), internalSlotSize);
            allocations.insert(std::make_pair((void*)(slot_ptr), internalSlotSize));
        }
#endif
        FatalAssert(slot_ptr->meta.in_use == 0, LOG_TAG_MEMORY, "Allocated slot is in use!");
        slot_ptr->meta.in_use = 1;
        slot_ptr->meta.type = static_cast<uint8_t>(SlotType::Internal);
        if (flags.clear) {
            std::memset(slot_ptr->data, 0, internalObjectSize);
        }

        return slot_ptr->data;
    }

    void* AllocateLeaf(AllocationFlags flags = {.value = 0}) {
        Slot* slot_ptr = nullptr;
        bool success = freeLeafSlots.TryPopHead(slot_ptr);
        if (!success) {
            slot_ptr = reinterpret_cast<Slot*>(nextAvailableLeafSlot.fetch_add(leafSlotSize));
            if (reinterpret_cast<uintptr_t>(slot_ptr) + leafSlotSize > leafEnd) {
            ...
            FatalAssert(slot_ptr->meta.type == static_cast<uint8_t>(SlotType::Leaf), LOG_TAG_MEMORY,
                        "Allocated slot type mismatch");
        } )
        ...
        {
            std::lock_guard<std::mutex> lock(allocation_mutex);
            AllocationSanityCheck((void*)(slot_ptr), leafSlotSize);
            allocations.insert(std::make_pair((void*)(slot_ptr), leafSlotSize));
        }
#endif
        FatalAssert(slot_ptr->meta.in_use == 0, LOG_TAG_MEMORY, "Allocated slot is in use!");
        slot_ptr->meta.in_use = 1;
        ...
        if (flags.clear) {
            std::memset(slot_ptr->data, 0, leafObjectSize);
        }
    ...
    void* Allocate(SlotType type, AllocationFlags flags = {.value = 0}) override {
        if (type == SlotType::Leaf) {
            return AllocateLeaf(flags);
        } else {
            return AllocateInternal(flags);
        }
    }

    size_t BatchAllocateInternal(void** ptr_array, size_t count,
        ...
        SANITY_CHECK(
            for (size_t i = 0; i < allocated; ++i) {
                FatalAssert(((Slot*)(ptr_array[i]))->meta.type == static_cast<uint8_t>(SlotType::Internal), LOG_TAG_MEMORY,
                            "Allocated slot type mismatch");
            }
        );
        ...
        if (allocated < count) {
            ...
            size_t available = (mid - nextInternal) / internalSlotSize;
            size_t to_allocate = std::min(available, count - allocated);
            if (to_allocate > 0) {
                uintptr_t begin = nextAvailableInternalSlot.fetch_add(to_allocate * internalSlotSize);
                uintptr_t end = begin + to_allocate * internalSlotSize;
                if (begin >= mid) {
                    // no available slots
                    begin = 0;
                    end = 0;
                    nextAvailableInternalSlot.fetch_sub(to_allocate * internalSlotSize);
                } else if (end > mid) {
                    size_t num_invalid = (end - mid + internalSlotSize - 1) / internalSlotSize;
                    nextAvailableInternalSlot.fetch_sub(num_invalid * internalSlotSize);
                    end -= num_invalid * internalSlotSize;
                }

                FatalAssert(end >= begin, LOG_TAG_MEMORY, "Invalid slot allocation range");
                FatalAssert((end - begin) % internalSlotSize == 0, LOG_TAG_MEMORY,
                            "Invalid slot allocation range alignment");
                size_t newly_allocated = (end - begin) / internalSlotSize;
        ...
        if (assigned > 0) {
            ...
            for (size_t i = 0; i < assigned; ++i) {
                ...
                {
                    AllocationSanityCheck((void*)(slot_ptr), internalSlotSize);
                    AllocationSanityCheck((void*)(slot_ptr), internalSlotSize, current_allocations);
                    current_allocations.insert(std::make_pair((void*)(slot_ptr), internalSlotSize));
                }
#endif
                FatalAssert(slot_ptr->meta.in_use == 0, LOG_TAG_MEMORY, "Allocated slot is in use!");
                slot_ptr->meta.type = static_cast<uint8_t>(SlotType::Internal);
            }
            freeInternalSlots.BatchPush(reinterpret_cast<Slot**>(ptr_array), assigned);
    ...
    size_t BatchAllocateLeaf(void** ptr_array, size_t count,
                             AllocationFlags flags = {.value = 0}) {
        CHECK_NOT_NULLPTR(ptr_array, LOG_TAG_MEMORY);
        FatalAssert(count > 0, LOG_TAG_MEMORY, "Count must be greater than 0");
        size_t allocated = 0;
        ...
        SANITY_CHECK(
            for (size_t i = 0; i < allocated; ++i) {
                FatalAssert(((Slot*)(ptr_array[i]))->meta.type == static_cast<uint8_t>(SlotType::Leaf), LOG_TAG_MEMORY,
                            "Allocated slot type mismatch");
            }
        );
        ...
        if (allocated < count) {
            ...
            size_t available = (leafEnd - nextLeaf) / leafSlotSize;
            size_t to_allocate = std::min(available, count - allocated);
            if (to_allocate > 0) {
                ...
                if (begin >= leafEnd) {
                    ...
                    size_t num_invalid = (end - leafEnd + leafSlotSize - 1) / leafSlotSize;
        ...
        if (assigned > 0) {
            ...
            for (size_t i = 0; i < assigned; ++i) {
                ...
                {
                    AllocationSanityCheck((void*)(slot_ptr), leafSlotSize);
                    AllocationSanityCheck((void*)(slot_ptr), leafSlotSize, current_allocations);
                    current_allocations.insert(std::make_pair((void*)(slot_ptr), leafSlotSize));
                }
#endif
                FatalAssert(slot_ptr->meta.in_use == 0, LOG_TAG_MEMORY, "Allocated slot is in use!");
                slot_ptr->meta.type = static_cast<uint8_t>(SlotType::Leaf);
    ...
    size_t BatchAllocate(SlotType type, void** ptr_array, size_t count,
        ...
        if (type == SlotType::Leaf) {
            return BatchAllocateLeaf(ptr_array, count, flags);
        } else {
            return BatchAllocateInternal(ptr_array, count, flags);
        }
    }
